...
const StyledCard = styled(Box)`
  background: ${({ theme }) => theme.colors.backgroundCardColor};
  border-radius: 8px;
  width: 100%;
  z-index: 1;
  padding: 1px;
...
const ProgressBase = styled(Box)<{cap: bigint, pos: bigint}>`
  ...
  border-radius: 8px;
  height: 16px;
  background: ${({ theme, cap, pos }) => cap >= pos ? theme.colors.backgroundAlt : theme.colors.primary66};
`

const ProgressBar = styled(Box)<{cap: bigint, pos: bigint}>`
  margin-top: 2px;
  border-radius: 8px;
  width: ${({ cap, pos }) => cap >= pos ? (Number(pos) / Number(cap) * 100).toString() : (Number(cap) / Number(pos) * (100)).toString()}%;
  border: 8px solid ${({ theme }) => theme.colors.primary};
`

const Badge = styled(Box)<{ status: string}>`
  background: ${({theme, status}) => status === "upcoming" ? theme.colors.primary3D : status === "live" || status === "success" ? theme.colors.success19 : theme.colors.text99};
  ...
  return (
      ...
      <Box p="20px">
        ...
        {data.presaleType === "standard" && searchBuyToken && <Box mb="0.5rem">
          ...
          <Text color="failure" fontSize="20px">{(Number(data.softCap) / Number(10**searchBuyToken.decimals)).toLocaleString()} {searchBuyToken.symbol} - {(Number(data.hardCap) / Number(10**searchBuyToken.decimals)).toLocaleString()} {searchBuyToken.symbol}</Text>
        </Box>}
        {data.presaleType === "fair" && searchBuyToken && <Box mb="0.5rem">
          <Text color="primary" fontSize="12px">Soft</Text>
          <Text color="failure" fontSize="20px">{(Number(data.softCap) / Number(10**searchBuyToken.decimals)).toLocaleString()} {searchBuyToken.symbol}</Text>
        </Box>}
        {searchBuyToken && <Box mb="0.5rem">
          <Text color="primary" fontSize="12px">Progress {`(${(Number(data.amount) / (data.presaleType === "standard" ? Number(data.hardCap) : Number(data.softCap)) * 100).toLocaleString()}%)`}</Text>
          ...
          <Flex justifyContent="space-between">
            <Text color="textDisabled" fontSize="16px">{(Number(data.amount) / (10**Number(searchBuyToken.decimals))).toLocaleString()} {searchBuyToken.symbol}</Text>
            <Text color="textDisabled" fontSize="16px">{data.presaleType === "standard" ? (Number(data.hardCap) / 10**Number(searchBuyToken.decimals)).toLocaleString() : (Number(data.softCap) / 10**Number(searchBuyToken.decimals)).toLocaleString()} {searchBuyToken.symbol}</Text>
          </Flex>
        </Box>}
        <Flex justifyContent="space-between">
