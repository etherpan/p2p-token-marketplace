...
import { getBlockExploreLink, getOnListSwapLink, getRouterName } from 'utils'
...
import KYCAndAuditModal from './components/KYCAndAuditModal'
...
const ProgressBase = styled(Box)<{cap: number, pos: number}>`
  ...
  border-radius: 8px;
  height: 16px;
  background: ${({ theme, cap, pos }) => cap >= pos ? theme.colors.text99 : theme.colors.primary66};
              ...
              <Box mt="15px">
                ...
                <Box>
                  ...
                  {isDataReady ? <Flex width="100%" justifyContent="space-between" px="5px" mb="10px">
                    ...
                    <Flex alignItems="center">
                      <LinkExternal href={getOnListSwapLink(data?.router, data?.token)}>
                        <Text small color="primary">{getRouterName(data?.router)}</Text>
                      </LinkExternal>
                      <CopyButton 
                        width="16px"
                        ml="5px"
                        buttonColor="textSubtle"
                        text={getOnListSwapLink(data?.router, data?.token)}
        ...
        >
          <StyledAppBody>
            <Box p="20px">
              ...
              </Flex> : !account ? <ConnectWalletButton /> : <Skeleton width="100%" height={60} />}
            </Box>
          </StyledAppBody>
          ...
          {isTokenReady && isBuyTokenReady && userDataLoaded && (data.userData?.owner ?? zeroAddress) && <StyledAppBody mt="16px">
            <Box p="20px">
              ...
              <Box mb="15px">
                <Text color="primary" fontSize="12px">KYC & Audit</Text>
                <Button
                  scale="sm"
                  variant="secondary"
                  width="100%"
                  mt="5px"
                  onClick={onKYCAndAudit}
                >
                  <Text fontSize="14px">Contact Support</Text>
                </Button>
              </Box>
              <Box mb="15px">
